...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning('You have not  create  any subjects yet! pls create one ')
        return
    
    subject_options = {f"{s['name']} - {s['join_code']}":s['subject_id'] for s in subjects }
...
def teacher_tab_attendance_record():
    ...
    summery['Attendance Stats']=(
        "✅"+summery['present_count'].astype(str)+"/"+summery['Total_Count'].astype(str) + ' studnets'
    )

    display_df = (summery.sort_values(by='ts_group',ascending=False)
